...
var url = 'https://www.gaopinpk10.com/pk10/kj?date='; //2018-04-09
...
getpk10data(new Date('2015-02-25'));
    ...
    request(options, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            $('#history tr').each(function (i, item) {
                ...
                var data = {};
                data._id = Number(_this.find('.p').text());
                data.date = formatdate(date);
                data.time = _this.find('.t').text();

                _this.find('.nums i').each(function (index, numberItem) {
                    data["num" + (index + 1)] = Number($(this).attr('class').replace('pk-no', ''));
                });
                dataArr.push(data);               
            });
